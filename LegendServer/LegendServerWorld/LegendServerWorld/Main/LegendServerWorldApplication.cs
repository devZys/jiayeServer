...
{
    public class LegendServerWorldApplication : ApplicationBase
    {
        public string ServerName = "";//服务器名称
        public int ServerID = 0;//服务器编号
        public List<int> DBServerList = new List<int>();//所有DB缓存服务器
#if RUNFAST
        ...
        protected override void Setup()
        {
            ...
            {
                ServerUtil.RecordLog(LogType.Fatal, "World读取本地配置出错！！！", new StackTrace(new StackFrame(true)).GetFrame(0));
                return;
            }
        ...
        public int DBServerID
        {
            get
            {
                if (DBServerList.Count <= 0) return 1;

                return DBServerList[LegendServer.Util.MyRandom.Next(DBServerList.Count)];
            }
        }
