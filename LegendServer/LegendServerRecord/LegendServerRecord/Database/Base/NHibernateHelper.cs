...
namespace LegendServer.Database
{
    ...
    public class NHibernateHelper
    {
        ...
        public static RunningCacheSenderInfo RunningCacheSender = new RunningCacheSenderInfo(false, 0, 0);
            ...
            try
            {
                lock (operateLock)
                {
                    using (var session = direct ? OpenDirectSession() : (typeof(T).ToString().Equals("LegendServer.Database.Profiler.ProfilerDB") ? OpenSpecialSession() : OpenNormalSession()))
                    {
                        try
                        {
                            if (RunningCacheSender.show)
                            {
                                ModuleManager.Get<ServiceBoxMain>().ShowRunningCache(typeof(T).ToString(), operate);
                            }
                            switch (operate)
                            {
        ...
        public static IList SQLSelect(string sql)
        {
            try
            {
                using (var session = OpenDirectSession())
                {
                    try
                    {
                        var playerList = session.CreateSQLQuery(sql);
                        session.Flush();
