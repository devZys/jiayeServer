...
namespace LegendServerAC
{
    ...
    public class InboundClientPeer : ClientPeer
    {
        public InboundClientPeer(InitRequest initRequest)
               : base(initRequest)
        {
            SessionManager.Instance.AddInboundClientSession(this.ConnectionId, ObjectPoolManager<InboundClientSession>.Instance.NewObject(this, initRequest.RemoteIP));
        }
        protected override void OnDisconnect(PhotonHostRuntimeInterfaces.DisconnectReason reasonCode, string reasonDetail)
        {
            int peerId = this.ConnectionId;
            ServerCPU.Instance.PushCommand(() => SessionManager.Instance.RemoveInboundClientSession(peerId));
        }
        ...
        public void OnClientSessionMsg(InboundClientSession session)
        {
            ...
            if ((currentDateTime - session.lastProcMsgTimeForUnitInterval).TotalMilliseconds <= ModuleManager.Get<SafeCheckMain>().procMsgAttackCheckTimePeriod)
            {
                ...
                if (session.procMsgCountByUnitInterval > ModuleManager.Get<SafeCheckMain>().procMsgCountLimitAttackCheck)
                {
                    ...
                    ServerUtil.RecordLog(LogType.Error, "检查到玩家IP：" + session.ip + " 恶意攻击，被锁定：" + session.frequentAttackWarningCount.ToString() + "次");

                    if (session.frequentAttackWarningCount >= ModuleManager.Get<SafeCheckMain>().procMsgAttackWarningCntLimit)
                    {
                        ServerUtil.RecordLog(LogType.Error, "检查到玩家IP：" + session.ip + " 恶意攻击超过限定次数,即将被断开连接");

                        session.peer.Disconnect();
                        SessionManager.Instance.RemoveInboundClientSession(session.peer.ConnectionId);
                    }
                }
            }
            else
