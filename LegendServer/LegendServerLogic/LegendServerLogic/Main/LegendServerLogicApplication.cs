...
{
    public class LegendServerLogicApplication : ApplicationBase
    {
        public string ServerName = "";//服务器名称
        public int ServerID = 0;//服务器编号
        public int DBServerID = 1;//被分配的DB缓存服务
#if MAHJONG
        ...
        {
            ServerUtil.RecordLog(LogType.Fatal, "存在非法连接，Logic服务器不允许有被动连接!!!", new StackTrace(new StackFrame(true)).GetFrame(0));
            return null;
        }
        protected override void Setup()
        {
            ...
            ServerInitialize.RegistSerialize();
            ...
            {
                ServerUtil.RecordLog(LogType.Fatal, "Logic读取本地配置出错！！！", new StackTrace(new StackFrame(true)).GetFrame(0));
                return;
            }
        ...
        {
            ModuleManager.Get<DistributedMain>().DBLoaded = true;
        }
        ...
        static public void StopService()
        {
            ...
            SessionManager.Instance.DisconnectAllProxy();
            SessionManager.Instance.DisconnectAllAC();
            SessionManager.Instance.DisconnectAllDB();
