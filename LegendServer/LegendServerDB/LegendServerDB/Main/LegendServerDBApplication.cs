...
{
    public class LegendServerDBApplication : ApplicationBase
    {
        public string ServerName = "";//服务器名称
        public int ServerID = 0;//服务器编号
        public List<int> AllLogicMappingMe = new List<int>();//所有分配到我的逻辑服务器
        ...
        protected override void Setup()
        {
            ...
            {
                ServerUtil.RecordLog(LogType.Fatal, "DB读取本地配置出错！！！", new StackTrace(new StackFrame(true)).GetFrame(0));
                return;
            }
        ...
        public void OnDBLoadCompleted()
        {
            ...
            ServerCPU.Instance.InitMQ(this);

            //处理注册的回调
            ModuleManager.Get<DistributedMain>().ProcessLoadDBCompletedCallBack();
        }
