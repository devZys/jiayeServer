...
namespace LegendProtocol
{
    ...
    public class ServerUtil
    {
        ...
        private static Dictionary<Type, IMessagePackSingleObjectSerializer> allObjectSerializer = new Dictionary<Type, IMessagePackSingleObjectSerializer>();
        public static void RegistSerializer(Type type)
        {
        ...
        public static byte[] Serialize(object obj)
        {
            try
            {
                ...
                if (!allObjectSerializer.ContainsKey(type)) return null;
                byte[] objByte = allObjectSerializer[type].PackSingleObject(obj);

                return objByte;
            }
            catch (Exception ex)
            {
                RecordLog(LogType.Error, ex, new StackTrace(new StackFrame(true)).GetFrame(0));
                return null;
            }
        }
        public static object UnSerialize(byte[] objByte, Type type)
        {
            try
            {
                if (objByte == null) return null;
                if (!allObjectSerializer.ContainsKey(type)) return null;
